...
exports.getAppointment=async (req,res)=>{
    ...
    const gotAppointment=await models.Appointment.findById(appointmentId).populate('type user','fullname email phonenumber city country title price length');
    if(!gotAppointment) return res.json({status:"error",error:"NO_APPOINTMENT"});
    const membership=await models.Member.find({user:gotAppointment.user._id});
